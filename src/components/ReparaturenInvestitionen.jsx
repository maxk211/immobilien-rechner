  ...
  return (
    <div className="bg-white border border-cream-200 rounded-lg p-4">
      <div className="flex justify-between items-center mb-4">
        <h3 className="font-semibold text-cream-800 flex items-center gap-1"><Wrench size={16} /> Reparaturen & Investitionen</h3>
        <button
          onClick={() => setShowForm(!showForm)}
          className="px-3 py-1 bg-clay-600 text-white text-sm rounded-lg hover:bg-clay-700"
        >
          + Hinzufügen
        </button>
      </div>
        ...
        <div className={`rounded-xl p-3 mb-4 border ${
          ...
          <div className="flex items-start justify-between gap-2 mb-2">
            <div>
              ...
              <div className={`text-xs mt-0.5 ${regel15.ueberschritten ? 'text-brick-700' : regel15.warnung ? 'text-honey-700' : 'text-clay-700'}`}>
                {regel15.ueberschritten
            ...
            <div className={`text-right shrink-0 text-xs font-bold ${regel15.ueberschritten ? 'text-brick-700' : regel15.warnung ? 'text-honey-700' : 'text-clay-700'}`}>
              {formatCurrency(regel15.relevantKosten)}<br />
              <span className="font-normal text-cream-500">/ {formatCurrency(regel15.grenze)}</span>
            </div>
          </div>
          {/* Fortschrittsbalken */}
          <div className="w-full bg-white/70 rounded-full h-2 overflow-hidden">
            <div
              className={`h-2 rounded-full transition-all duration-700 ${regel15.ueberschritten ? 'bg-brick-500' : regel15.warnung ? 'bg-honey-400' : 'bg-clay-500'}`}
      ...
      {showForm && (
        <div className="bg-cream-50 p-3 rounded-lg mb-4">
          <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            <div>
              <label className="block text-xs text-cream-600 mb-1">Kategorie</label>
              <select
          ...
          <div className="mb-3">
            <label className="block text-xs text-cream-600 mb-1">Betrag (€)</label>
            <input
              type="number"
          ...
          <div className="flex gap-2">
            <button onClick={handleAdd} className="px-3 py-1 bg-sage-600 text-white text-sm rounded hover:bg-sage-700">
              Speichern
            </button>
            <button onClick={() => setShowForm(false)} className="px-3 py-1 bg-cream-300 text-cream-700 text-sm rounded hover:bg-cream-400">
              Abbrechen
            </button>
          </div>
        </div>
      ...
      {investitionen.length > 0 && (
        <div className="bg-cream-50 p-3 rounded-lg mb-4">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-lg font-bold text-honey-600">{formatCurrency(gesamtInvestitionen)}</span>
          </div>
      ...
      <div className="overflow-y-auto">
        {investitionen.length === 0 ? (
          <div className="text-center text-cream-400 py-4 text-sm">
            Noch keine Reparaturen oder Investitionen erfasst
          </div>
        ) : (
          <div className="space-y-2">
            {investitionen.sort((a, b) => new Date(b.datum) - new Date(a.datum)).map(inv => (
              <div key={inv.id} className="flex justify-between items-center p-2 bg-cream-50 rounded">
                <div className="flex items-center gap-2">
                  <span>{kategorien[inv.kategorie]?.icon}</span>
                  <div>
                    <div className="text-sm font-medium">{inv.beschreibung}</div>
                    <div className="text-xs text-cream-500">{new Date(inv.datum).toLocaleDateString('de-DE')}</div>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <span className="font-semibold text-honey-600">{formatCurrency(inv.betrag)}</span>
                  <button
