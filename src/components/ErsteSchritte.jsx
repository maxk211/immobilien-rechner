...
export default function ErsteSchritte({ portfolio, mieterListe }) {
  ...
  return (
    <div className="mb-6 bg-gradient-to-br from-clay-50 to-white border border-clay-100 rounded-2xl px-4 py-3.5 sm:px-5 sm:py-4 relative">
      <button
        onClick={schliessen}
        className="absolute top-3 right-3 text-cream-300 hover:text-cream-500 transition-colors"
      ...
      <div className="flex items-center gap-2 mb-3 pr-6">
        {alleErledigt ? <Sparkles size={16} className="text-clay-500" /> : null}
        <h3 className="text-sm font-semibold text-cream-800">
          {alleErledigt ? 'Startklar — alle ersten Schritte erledigt!' : 'Erste Schritte'}
        </h3>
        <span className="text-xs text-cream-400 font-medium">{erledigtCount}/{schritte.length}</span>
      </div>
        ...
        {schritte.map(s => (
          <div key={s.id} className="flex items-center gap-1.5">
            {s.erledigt
              ...
              : <Circle size={16} className="text-cream-300 flex-shrink-0" />}
            <span className={`text-xs sm:text-[13px] ${s.erledigt ? 'text-cream-400 line-through' : 'text-cream-600 font-medium'}`}>
              {s.label}
            </span>
          </div>
        ))}
