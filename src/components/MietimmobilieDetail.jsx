    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-cream-500 mt-0.5">Mietvertrag, Untermietverträge & alle weiteren Unterlagen</p>
        </div>
      ...
      <div
          ...
          {ARB_DOK_TYPEN.map(t => (
            <button key={t} onClick={() => setGewaehltTyp(t)}
              className={`text-xs px-2.5 py-1 rounded-full font-semibold transition-all ${
                gewaehltTyp === t ? 'bg-sage-600 text-white shadow-sm' : 'bg-cream-100 text-cream-600 hover:bg-cream-200'
              }`}>
              {t}
            </button>
          ))}
        ...
        <label className={`flex flex-col items-center justify-center gap-2 cursor-pointer py-2 ${uploading ? 'opacity-50 pointer-events-none' : ''}`}>
          <div className="w-12 h-12 rounded-full bg-sage-50 flex items-center justify-center">
            {uploading ? <Loader2 size={24} className="animate-spin text-sage-400"/> : <Upload size={24} className="text-sage-400"/>}
          </div>
          <p className="text-sm font-semibold text-cream-700">{uploading ? 'Wird hochgeladen…' : 'Datei hochladen'}</p>
          <p className="text-xs text-cream-400">{uploading ? 'Bitte warten' : 'Klicken oder Datei hierher ziehen · max. 20 MB'}</p>
        ...
        {uploadFehler && (
          <div className="text-xs text-brick-600 bg-brick-50 rounded-lg px-3 py-2 text-center flex items-center justify-center gap-1"><AlertTriangle size={14}/>{uploadFehler}</div>
        )}
      </div>
      ...
      {dokumente.length === 0 ? (
        <div className="text-center py-10 text-cream-400">
          <FolderOpen size={36} className="mx-auto mb-2 text-cream-300"/>
          ...
          {gruppen.map(({ typ, liste }) => (
            <div key={typ}>
              <div className="flex items-center gap-2 mb-2">
                <FileText size={14} className="text-cream-400"/>
                <span className="text-xs font-bold text-cream-600 uppercase tracking-wide">{typ}</span>
                <span className="text-xs text-cream-400">({liste.length})</span>
              </div>
              <div className="space-y-1.5">
                {[...liste].reverse().map(doc => (
                  <div key={doc.id} className="flex items-center gap-3 bg-white border border-cream-200 rounded-xl px-3 py-2.5 hover:border-sage-200 hover:shadow-sm transition-all group">
                    <FileText size={16} className="flex-shrink-0 text-cream-400"/>
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      <button onClick={() => handleDownload(doc)} disabled={ladeId === doc.id}
                        className="p-1.5 rounded-lg text-sage-600 hover:bg-sage-50 disabled:opacity-50" title="Öffnen">
                        {ladeId === doc.id ? <Loader2 size={16} className="animate-spin"/> : <Download size={16}/>}
                      </button>
                      <button onClick={() => handleDelete(doc)}
                        className="p-1.5 rounded-lg text-brick-400 hover:bg-brick-50 opacity-0 group-hover:opacity-100 transition-opacity" title="Löschen">
                        <Trash2 size={16}/>
                      </button>
                    </div>
                  </div>
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex flex-col justify-end sm:flex-row sm:items-center sm:justify-center sm:p-4">
      <div className="bg-white w-full rounded-t-3xl sm:rounded-2xl shadow-2xl sm:max-w-5xl h-[93vh] sm:h-[95vh] flex flex-col overflow-hidden">
        ...
        <div className="flex-shrink-0 overflow-hidden">
          <div className="bg-gradient-to-r from-sage-500 to-sage-700 px-4 sm:px-6 pt-4 sm:pt-5 pb-3 sm:pb-4">
            <div className="flex justify-between items-start">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  {vertragsBeendet && (
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-brick-500/80 text-white flex items-center gap-1"><span className="inline-block w-2 h-2 rounded-full bg-brick-300"/> Vertrag beendet {new Date(params.mietvertragEnde).toLocaleDateString('de-DE')}</span>
                  )}
                  {vertragsende && !vertragsBeendet && (
                    <span className="text-xs font-semibold px-2 py-0.5 rounded-full bg-honey-400/80 text-honey-900 flex items-center gap-1"><Loader2 size={11}/> Endet {new Date(params.mietvertragEnde).toLocaleDateString('de-DE')}</span>
                  )}
                </div>
                <h2 className="text-lg sm:text-2xl font-black text-white truncate">{params.name || 'Mietimmobilie'}</h2>
                {(params.plz || params.adresse) && (
                  <p className="text-sage-100 text-sm mt-0.5 flex items-center gap-1"><MapPin size={12}/> {params.plz} {params.adresse}</p>
                )}
              </div>
              <div className="flex items-center gap-2 ml-4 shrink-0">
                ...
                {hasChanges && (
                  <button onClick={handleSave}
                    className="px-4 py-2 bg-white text-sage-700 rounded-xl hover:bg-sage-50 font-bold text-sm shadow-sm transition-colors">
                    Speichern
                  </button>
                )}
<button onClick={onClose} className="w-8 h-8 flex items-center justify-center text-white/60 hover:text-white"><X size={20}/></button>
          ...
          {/* KPI Strip */}
          <div className="grid grid-cols-3 bg-white border-b border-cream-200 divide-x divide-cream-100">
            <div className="px-2 sm:px-5 py-2 sm:py-3">
              <div className="text-[10px] sm:text-xs text-cream-400 font-medium uppercase tracking-wide">Monatl.</div>
              <div className={`text-sm sm:text-xl font-black ${monatsCashflow >= 0 ? 'text-sage-600' : 'text-brick-600'}`}>
                {monatsCashflow >= 0 ? '+' : ''}{formatCurrency(monatsCashflow)}
              </div>
            </div>
            <div className="px-2 sm:px-5 py-2 sm:py-3">
              <div className="text-[10px] sm:text-xs text-cream-400 font-medium uppercase tracking-wide">Jährlich</div>
              <div className={`text-sm sm:text-xl font-black ${jahresCashflow >= 0 ? 'text-sage-600' : 'text-brick-600'}`}>
                {jahresCashflow >= 0 ? '+' : ''}{formatCurrency(jahresCashflow)}
              </div>
            </div>
            <div className="px-2 sm:px-5 py-2 sm:py-3">
              <div className="text-[10px] sm:text-xs text-cream-400 font-medium uppercase tracking-wide">Bisher gesamt</div>
              <div className="text-sm sm:text-xl font-black text-sage-700">
                {bisherigeCashflowGesamt >= 0 ? '+' : ''}{formatCurrency(bisherigeCashflowGesamt)}
              </div>
              <div className="text-[10px] sm:text-xs text-cream-400">{monateSeitStart} Mo.</div>
            </div>
          </div>
          {/* Tab-Navigation */}
          <div className="overflow-x-auto flex-shrink-0">
            <div className="flex gap-1 bg-cream-100 p-1 min-w-max">
              {[
                ...
                <button key={tab.id} onClick={() => setActiveTab(tab.id)}
                  className={`py-2 px-3 sm:px-4 text-[11px] sm:text-sm font-semibold rounded-lg transition-all whitespace-nowrap ${activeTab === tab.id ? 'bg-white text-sage-700 shadow-sm' : 'text-cream-500 hover:text-cream-800'}`}>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 p-3 sm:p-6">
          ...
          {activeTab === 'mieteingaenge' && (() => {
            ...
            return (
              <div>
                <div className="mb-4 bg-sage-50 border border-sage-200 rounded-xl p-3 flex items-start gap-2">
                  <Lightbulb size={16} className="text-sage-700 flex-shrink-0"/>
          ...
          <div className="bg-cream-50 rounded-2xl border border-cream-200 p-5 mb-6">
            <h3 className="text-sm font-bold text-cream-600 uppercase tracking-wide mb-4">Cashflow-Aufschlüsselung</h3>
            <div className="space-y-1">
              <div className="flex justify-between items-center py-2.5 border-b border-cream-200">
                ...
                <span className="text-sm font-bold text-sage-600">+{formatCurrency(einnahmen)}</span>
              </div>
              <div className="flex justify-between items-center py-2.5 border-b border-cream-200">
                <span className="text-sm text-cream-600">Eigene Warmmiete</span>
                <span className="text-sm font-bold text-brick-500">−{formatCurrency(aktWarmmiete)}</span>
              </div>
              {zusatzkosten > 0 && (
                <div className="flex justify-between items-center py-2.5 border-b border-cream-200">
                  <span className="text-sm text-cream-600">
                  ...
                  <span className="text-sm font-bold text-brick-500">−{formatCurrency(zusatzkosten)}</span>
                </div>
              )}
              <div className={`flex justify-between items-center pt-3 font-black text-base ${monatsCashflow >= 0 ? 'text-sage-600' : 'text-brick-600'}`}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white border border-cream-200 rounded-2xl p-5 shadow-sm">
              <h3 className="text-sm font-bold text-cream-500 uppercase tracking-wide mb-4 flex items-center gap-1"><MapPin size={14}/> Grunddaten</h3>
              <div className="space-y-3">
                <div>
                  <label className="block text-sm text-cream-600 mb-1">Name/Bezeichnung</label>
                  <input
                    type="text"
                    value={params.name}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm text-cream-600 mb-1">PLZ</label>
                    <input
                      type="text"
                      value={params.plz}
                  ...
                  <div>
                    <label className="block text-sm text-cream-600 mb-1">Mietvertrag seit</label>
                    <input
                      type="date"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm text-cream-600 mb-1">Mietvertragsende</label>
                    <input
                      type="date"
                      value={params.mietvertragEnde}
                      onChange={(e) => updateParams({ mietvertragEnde: e.target.value })}
                      className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-clay-500 ${vertragsBeendet ? 'border-brick-400 bg-brick-50' : 'border-cream-300'}`}
                    />
                    {params.mietvertragEnde && (
                      <button
                        type="button"
                        onClick={() => updateParams({ mietvertragEnde: '' })}
                        className="text-xs text-cream-400 hover:text-brick-500 mt-1 flex items-center gap-1"
                      >
                  ...
                  <div className="flex items-end pb-2">
                    {vertragsBeendet && (
                      <div className="text-xs text-brick-600 bg-brick-50 border border-brick-200 rounded p-2">
                        Cashflow wird ab Vertragsende nicht mehr berechnet.
                      </div>
                    )}
                    {vertragsende && !vertragsBeendet && (
                      <div className="text-xs text-honey-700 bg-honey-50 border border-honey-200 rounded p-2">
                ...
                <div>
                  <label className="block text-sm text-cream-600 mb-1">Adresse</label>
                  <input
                    type="text"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-sm text-cream-600 mb-1">Wohnfläche (m²)</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="block text-sm text-cream-600 mb-1">Gesamtzahl Zimmer</label>
                    <input
                      type="number"
                      value={params.zimmer}
            ...
            {/* Finanzdaten */}
            <div className="bg-white border border-cream-200 rounded-2xl p-5 shadow-sm">
              <h3 className="text-sm font-bold text-cream-500 uppercase tracking-wide mb-4 flex items-center gap-1"><Wallet size={14}/> Arbitrage-Kalkulation</h3>
              <div className="space-y-4">
                <div className="p-3 bg-brick-50 rounded-lg border border-brick-100">
                  <label className="block text-sm font-medium text-brick-700 mb-1">Eigene Warmmiete (€/Monat)</label>
                  <input
                    type="number"
                    value={params.eigeneWarmmiete}
                    onChange={(e) => updateParams({ eigeneWarmmiete: parseFloat(e.target.value) || 0 })}
                    className="w-full px-3 py-2 border border-brick-300 rounded-lg focus:ring-2 focus:ring-brick-500 text-lg font-semibold"
                  />
                  <p className="text-xs text-brick-600 mt-1">Die Miete, die du an den Vermieter zahlst</p>
                </div>

                <div className="p-3 bg-sage-50 rounded-lg border border-sage-100">
                  <label className="block text-sm font-medium text-sage-700 mb-2">Untervermietung</label>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-xs text-cream-600 mb-1">Vermietete Zimmer</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-xs text-cream-600 mb-1">Miete pro Zimmer (€)</label>
                      <input
                        type="number"
                  ...
                  <p className="text-xs text-sage-600 mt-2">
                    ...
                    {aktUntermiete !== params.untermieteProZimmer && (
                      <span className="text-cream-400 ml-1">(Basis: {formatCurrency(params.untermieteProZimmer)} → Anpassung aktiv)</span>
                    )}
                  </p>
                </div>

                <div className="p-3 bg-cream-50 rounded-lg border border-cream-200">
                  <label className="block text-sm font-medium text-cream-700 mb-2 flex items-center gap-1"><Settings size={14}/> Zusätzliche Kosten (für Steuerberater)</label>
                  <div className="grid grid-cols-3 gap-2">
                    <div>
                      <label className="block text-xs text-cream-500 mb-1 flex items-center gap-1"><Zap size={11}/> Strom</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-xs text-cream-500 mb-1 flex items-center gap-1"><Globe size={11}/> Internet</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-xs text-cream-500 mb-1">GEZ</label>
                      <input
                        type="number"
                ...
                <div className="p-3 bg-honey-50 rounded-lg border border-honey-200">
                  <div className="flex justify-between items-center mb-2">
                    <label className="text-sm font-medium text-honey-800 flex items-center gap-1"><CalendarDays size={13}/> Mietanpassungen</label>
                    <button
                      type="button"
                      onClick={() => {
                      ...
                      className="text-xs bg-honey-200 hover:bg-honey-300 text-honey-800 px-2 py-1 rounded"
                    >
                      + Anpassung
                    </button>
                  </div>
                  <p className="text-[10px] text-honey-700 mb-2">Trage Änderungen an deiner Miete oder Untermiete mit Datum ein – wird für den korrekten Steuerexport verwendet.</p>
                  {(params.mietAnpassungen || []).length === 0 ? (
                    <p className="text-[10px] text-cream-400 italic bg-white p-2 rounded border border-honey-100">Keine Anpassungen → aktuelle Werte gelten durchgehend</p>
                  ) : (
                    <div className="space-y-2">
                      {(params.mietAnpassungen || [])
                        ...
                        .map((anp) => (
                          <div key={anp.originalIdx} className="bg-white rounded border border-honey-200 p-2">
                            <div className="flex items-center gap-2 mb-1.5">
                              <span className="text-[10px] text-cream-500 w-10 shrink-0">Datum</span>
                              <input
                                type="date"
                                value={anp.datum}
                                onChange={(e) => {
                                ...
                                className="text-xs border border-cream-300 rounded px-1 py-0.5 flex-1"
                              />
                              <button
                                type="button"
                                onClick={() => {
                                ...
                                className="text-brick-400 hover:text-brick-600 px-1 shrink-0"
                              >
                                <X size={12}/>
                              </button>
                            </div>
                            <div className="grid grid-cols-2 gap-2">
                              <div className="flex items-center gap-1">
                                <span className="text-[10px] text-brick-600 shrink-0">Warmmiete</span>
                                <input
                                  type="number"
                                  ...
                                  className="w-full text-xs border border-brick-200 rounded px-1 py-0.5 text-right"
                                />
                                <span className="text-[10px] text-cream-400">€</span>
                              </div>
                              <div className="flex items-center gap-1">
                                <span className="text-[10px] text-sage-600 shrink-0">Untermiete</span>
                                <input
                                  type="number"
                                  ...
                                  className="w-full text-xs border border-sage-200 rounded px-1 py-0.5 text-right"
                                />
                                <span className="text-[10px] text-cream-400">€</span>
                              </div>
                            </div>
          ...
          {/* Prognose */}
          <div className="mt-5 bg-white border border-cream-200 rounded-2xl p-5 shadow-sm">
            <h3 className="text-sm font-bold text-cream-500 uppercase tracking-wide mb-4 flex items-center gap-1"><TrendingUp size={14}/> Kumulierter Cashflow</h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[[1,'1 Jahr'],[2,'2 Jahre'],[3,'3 Jahre'],[5,'5 Jahre']].map(([mult, label]) => (
                <div key={mult} className={`rounded-xl p-4 text-center border ${jahresCashflow >= 0 ? 'bg-sage-50 border-sage-100' : 'bg-brick-50 border-brick-100'}`}>
                  <div className="text-xs text-cream-400 font-medium mb-1">{label}</div>
                  <div className={`text-lg font-black ${jahresCashflow >= 0 ? 'text-sage-600' : 'text-brick-600'}`}>
