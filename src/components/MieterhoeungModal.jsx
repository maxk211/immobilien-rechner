    ...
    <div className="fixed inset-0 bg-black/60 z-[70] flex flex-col justify-end sm:items-center sm:justify-center sm:p-4">
      <div className="bg-white w-full rounded-t-3xl sm:rounded-2xl shadow-2xl sm:max-w-lg max-h-[95vh] flex flex-col">
        ...
        <div className="sm:hidden flex justify-center pt-2.5 pb-1 flex-shrink-0">
          <div className="w-10 h-1.5 bg-cream-200 rounded-full" />
        </div>

        {/* Header */}
        <div className="px-5 pt-4 pb-3 border-b border-cream-100 flex-shrink-0 flex items-start justify-between">
          <div>
            <h2 className="text-lg font-black text-cream-900 flex items-center gap-2"><TrendingUp size={20} /> Mieterhöhung</h2>
            <p className="text-xs text-cream-500 mt-0.5">{effMieterName ? `${effMieterName} · ` : ''}{immobilie?.name || immobilie?.adresse || 'Ohne Zuordnung'}</p>
          </div>
          <button onClick={onClose} className="text-cream-400 hover:text-cream-700 leading-none"><X size={20} /></button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {!hatMieter && (
            <div className="bg-honey-50 rounded-xl p-4 border border-honey-200">
              <p className="text-xs font-semibold text-honey-700 mb-3 uppercase tracking-wide flex items-center gap-1.5"><User size={14} /> Mieter-Angaben (für das Schreiben)</p>
              <div className="space-y-2">
                <input type="text" value={form.mieterName}
                  onChange={e => setForm({ ...form, mieterName: e.target.value })}
                  className="w-full px-3 py-2 border border-honey-300 rounded-lg text-sm bg-white"
          ...
          <div className="bg-clay-50 rounded-xl p-4 border border-clay-100">
            <p className="text-xs font-semibold text-clay-700 mb-3 uppercase tracking-wide">Mietänderung</p>
            <div className="flex items-center gap-3">
              <div className="flex-1">
                <p className="text-[10px] text-cream-400 mb-1">Aktuelle Kaltmiete</p>
                {hatMieter ? (
                  <p className="text-xl font-black text-cream-500 line-through">{formatCurrency(effAktKaltmiete)}</p>
                ) : (
                  <input
                    type="number"
                    ...
                    className="w-full px-3 py-2 border-2 border-cream-300 rounded-lg text-xl font-black text-cream-500 text-right focus:outline-none focus:ring-2 focus:ring-cream-400"
                    min={0} step={5} placeholder="0"
                  />
                )}
              </div>
              <span className="text-2xl text-cream-300">→</span>
              <div className="flex-1">
                <p className="text-[10px] text-cream-400 mb-1">Neue Kaltmiete</p>
                <input
                  type="number"
                  value={form.neueKaltmiete}
                  onChange={e => setForm({ ...form, neueKaltmiete: parseFloat(e.target.value) || '' })}
                  className="w-full px-3 py-2 border-2 border-clay-400 rounded-lg text-xl font-black text-clay-700 text-right focus:outline-none focus:ring-2 focus:ring-clay-500"
                  min={0}
                  step={5}
                />
              </div>
            ...
            {neueKalt > 0 && effAktKaltmiete > 0 && (
              <div className={`mt-3 flex justify-between items-center text-sm font-semibold px-1 ${diff > 0 ? 'text-sage-600' : 'text-brick-600'}`}>
                <span>Differenz:</span>
                <span>{diff > 0 ? '+' : ''}{formatCurrency(diff)}/mo ({diffProzent} %)</span>
              </div>
            )}
            {diff / effAktKaltmiete > 0.2 && effAktKaltmiete > 0 && (
              <p className="mt-2 text-xs text-brick-600 bg-brick-50 rounded-lg px-3 py-2 flex items-start gap-1.5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-semibold text-cream-600 mb-1">Wirksamkeitsdatum <span className="text-cream-400 font-normal">(§ 558b BGB)</span></label>
              <input
                type="date"
              ...
              />
              <p className="text-[10px] text-cream-400 mt-1">Frühestens {new Date(frühestesWirksamkeitsDatum).toLocaleDateString('de-DE')}</p>
            </div>
            <div>
              <label className="block text-xs font-semibold text-cream-600 mb-1">Zustimmungsfrist Mieter</label>
              <div className="px-3 py-2 bg-cream-50 border border-cream-200 rounded-lg text-sm font-semibold text-cream-700">
                {new Date(zustimmungsFrist).toLocaleDateString('de-DE')}
              </div>
              <p className="text-[10px] text-cream-400 mt-1">Ende des 2. Monats nach Zugang</p>
            </div>
          </div>

          {/* Begründung */}
          <div>
            <label className="block text-xs font-semibold text-cream-600 mb-2">Begründung (§ 558a BGB)</label>
            <div className="space-y-2">
              {begründungsOptionen.map(opt => (
                <label key={opt.value} className={`flex items-start gap-3 p-3 rounded-xl border-2 cursor-pointer transition-all ${form.begründungsTyp === opt.value ? 'border-clay-400 bg-clay-50' : 'border-cream-200 bg-white hover:border-cream-300'}`}>
                  ...
                  <div>
                    <p className="text-sm font-semibold text-cream-800 flex items-center gap-1.5">{opt.icon}{opt.label}</p>
                    <p className="text-xs text-cream-500">{opt.desc}</p>
                  </div>
                </label>
              ))}
            </div>
            ...
            {form.begründungsTyp === 'mietspiegel' && (
              <div className="mt-3 grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-cream-500 mb-1">Mietspiegel-Jahr</label>
                  <input type="text" value={form.mietspiegelJahr}
                    onChange={e => setForm({ ...form, mietspiegelJahr: e.target.value })}
                    className="w-full px-3 py-2 border border-cream-300 rounded-lg text-sm" placeholder="z.B. 2025" />
                </div>
                <div>
                  <label className="block text-xs text-cream-500 mb-1">Quelle</label>
                  <input type="text" value={form.mietspiegelQuelle}
                    onChange={e => setForm({ ...form, mietspiegelQuelle: e.target.value })}
                    className="w-full px-3 py-2 border border-cream-300 rounded-lg text-sm" placeholder="Mietspiegel der Gemeinde" />
                </div>
              </div>
            )}

            <div className="mt-3">
              <label className="block text-xs text-cream-500 mb-1">Ergänzung (optional)</label>
              <textarea value={form.eigenerText}
                onChange={e => setForm({ ...form, eigenerText: e.target.value })}
                className="w-full px-3 py-2 border border-cream-300 rounded-lg text-sm resize-none"
          ...
          <div className="bg-cream-50 rounded-xl p-4 border border-cream-200">
            <p className="text-xs font-semibold text-cream-600 mb-3 uppercase tracking-wide">Vermieter-Angaben (für das Schreiben)</p>
            <div className="space-y-2">
              <input type="text" value={form.vermieterName}
                onChange={e => setForm({ ...form, vermieterName: e.target.value })}
                className="w-full px-3 py-2 border border-cream-300 rounded-lg text-sm" placeholder="Vorname Nachname" />
              <input type="text" value={form.vermieterStrasse}
                onChange={e => setForm({ ...form, vermieterStrasse: e.target.value })}
                className="w-full px-3 py-2 border border-cream-300 rounded-lg text-sm" placeholder="Straße Hausnummer" />
              <input type="text" value={form.vermieterPlzOrt}
                onChange={e => setForm({ ...form, vermieterPlzOrt: e.target.value })}
                className="w-full px-3 py-2 border border-cream-300 rounded-lg text-sm" placeholder="12345 Musterstadt" />
            </div>
          </div>
        </div>

        {/* Footer Actions */}
        <div className="p-4 border-t border-cream-100 flex-shrink-0 space-y-2">
          <button
            onClick={generatePDF}
            disabled={!neueKalt || neueKalt <= 0}
            className="w-full py-3 bg-cream-800 text-white rounded-xl text-sm font-bold hover:bg-cream-900 disabled:opacity-40 flex items-center justify-center gap-2"
          >
            <FileText size={16} /> Mieterhöhungsschreiben als PDF
          </button>
          <div className="flex gap-2">
            <button onClick={onClose} className="flex-1 py-3 bg-cream-100 rounded-xl text-sm font-semibold text-cream-700">
              Abbrechen
            </button>
            <button
              onClick={handleSave}
              disabled={!neueKalt || neueKalt <= 0}
              className="flex-1 py-3 bg-sage-500 text-white rounded-xl text-sm font-bold hover:bg-sage-600 disabled:opacity-40 flex items-center justify-center gap-2"
            >
